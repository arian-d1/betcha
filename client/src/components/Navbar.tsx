      ...
      <div className="flex items-center gap-4">
        ...
        >
          <Button variant="ghost" size="sm" className="gap-2">
            {auth.isAuthenticated && auth.user?.username ? null : <User2 className="h-4 w-4" />}
            <span className="hidden sm:inline">{auth.isAuthenticated && auth.user?.username ? `@${auth.user.username}` : 'Login'}</span>
          </Button>
        </Link>
